...
class DatasetReading:
    def get_file(self):
        ...
        sl.write("<h1 style='text-align:Center'>Automated Pre-Processing</h1>",unsafe_allow_html=True)
...
class DataHandeling(DatasetReading):
    ...
    def save_file(self):
        ...
        self.get_df_info()
        if '.csv' in self.file:
            self.dataset.to_csv(f"{self.file_name} Processed.csv",index=True)

            return self.dataset
        elif '.xlsx' in self.file:
            self.dataset.to_excel(f'{self.file_name} Processed.xlsx',index=True)
            return self.dataset
